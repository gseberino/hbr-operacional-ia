...
try {
  ...
  for (const expected of ['Copiar p/ grupo', 'Copiar relatorio', 'Relatorio PDF', 'Agenda PDF', 'Kanban PDF', 'Rendimento do dia', 'Carga por responsavel', 'Filtros', 'filter_responsible', 'filter_kanban_group', 'filter_planning_view', 'filter_saved_view_id', 'Salvar view atual', 'Filtros avancados', 'Central de Planejamento', 'planning-board', 'Por responsavel', 'Responsaveis', 'Periodos do dia', 'Meta diaria de tarefas', 'Analise operacional', 'Plano do dia', 'Preencher com modelo', 'data-template-select', 'Resumo dos vinculos', 'Modelos de preenchimento de tarefa', 'drawer-tabs', 'Prazo e importancia', 'Contexto e bloqueios', 'Checklist e subtarefas', 'IA e automacao', 'Acoes e historico', 'Campos complementares', 'Mais dados', 'Dados do cadastro', 'Campos customizados', 'Automacoes internas', 'Score operacional', 'data-kanban-status', 'data-kanban-responsible', 'data-edit-task', 'data-drawer-draft-email', 'data-drawer-draft-whatsapp', 'data-drawer-timer-start', 'data-drawer-timer-pause', 'Conferir timer a cada', 'Como o Agente IA trabalha', 'settingsForm', 'Passo do score', 'Passo do esforço', 'Salvar configuracoes']) {
  ...
  assert(appJs.includes("const appVersion = '0.3.7'"), 'Versao visivel do app nao foi atualizada.');
  const version = await request('/api/app-version', { headers: { Cookie: cookie } });
  assert(version.data.version === '0.3.7', 'Backend nao reportou versao 0.3.7.');
